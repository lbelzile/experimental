...
Given the Brown-Forsythe test output, we can 
try fitting a different variance in each group, as there are enough observations for this. The function `gls` in the `nlme` package fits such models; the weight argument being setup with a constant variance (`~1`) per each combination of the crossed factors `anchor * magnitude`.
...
# Note the unequal std. deviation per group
emmeans(object = mod2, 
        specs = c("anchor", "magnitude"))
...
Regardless of the model, it should be clearly stated that there is some evidence of hetegeneity. We should also report sample size per group, mention the repartition ($n=30$ per group).
...
For the experiment to make sense, we should check that travel time or distance is indeed roughly the same between each station. The limited range of the Likert scale shows
These hypotheses can be expressed in terms of contrasts.
