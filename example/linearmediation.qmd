...
$$
M = \underset{\text{intercept}}{c_M} + \alpha X + \underset{\text{control variates}}{g_M(\boldsymbol{Z})} + \underset{\text{error term}}{\varepsilon_M}
$$ {#eq-mediator-model}
while we have for the response
$$
Y = \underset{\text{intercept}}{c_Y} + \underset{\text{direct effect}}{\beta X} + \gamma M + \underset{\text{control variates}}{g_Y(\boldsymbol{Z})}  + \underset{\text{error term}}{\varepsilon_Y}, \label{eq-responsemodel}
$$ {#eq-response-model}
The explanatory variables $\boldsymbol{Z}$  are control covariates to account for confounders, and need not be the same in both models (but could be), depending on the functional form $g_M(\cdot)$ and $g_Y(\cdot)$.  The error terms $\varepsilon_Y$ and $\varepsilon_M$ are assumed to be **independent** mean-zero errors terms, each with constant variance.
...
\node [draw,rectangle,fill=black!10] (Zy) at (2,1) {$\boldsymbol{Z}$};
\draw [-latex] (X) edge (Y);
\draw [-latex] (M) edge (Y);
\draw [-latex] (X) edge (M);
...
$$\begin{align*}
\mathsf{E}[Y_i\{x, M_i(1)\}]  &= \underset{\text{intercept}}{c_Y + \gamma c_M} + \underset{\text{ACME}}{\alpha\gamma} + \beta x+ \underset{\text{effect of control variates}}{\gamma g_M(\boldsymbol{Z})  + g_Y(\boldsymbol{Z})}\\
\mathsf{E}[Y_i\{x, M_i(0)\}]  &=\underset{\text{intercept}}{c_Y + \gamma c_M} + \beta x +\underset{\text{effect of control variates}}{\gamma g_M(\boldsymbol{Z})  + g_Y(\boldsymbol{Z})}\\
\end{align*}
$$
