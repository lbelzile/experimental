...
We consider for simplicity a bivariate contingency table with $I$ rows and $J$ columns; the count in the $(i,j)$ entry of the matrix is $n_{ij}$ and there are $n$ observations overall.


Pearson's $X^2$ goodness of fit test examines discrepancies between  postulated proportions in each cell (with the sum of the probabilities summing to one, $\sum_{i=1}^I \sum_{j=1}^J p_{ij0}=1$). The test compare the expected counts $E_{ij}=np_{ij0}$ with the observed counts $O_{ij}=n_{ij}$. As summary of evidence, we take
$$ X^2 = \sum_{i,j} \frac{(E_{ij}-O_{ij})^2}{E_{ij}},$$
the squared difference between expected and observed counts, divided by the expected counts.^[The square root of the summands represents the so-called Pearson residuals.]

The contingency table described above represents a two-way factorial design on which we impose $IJ-1$ constraints. As in any experiment, there are logical comparisons that could be made.
...
We can check that our summaries match those reported by the authors.
