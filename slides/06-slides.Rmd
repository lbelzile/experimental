...
model <- afex::aov_ez(
  ...
  within = "stimulus", # within-subject
  data = hecedsm::AA21,
  fun_aggregate = mean) # if repeated measurements
```
]
]
.pull-right[
...
> We conducted a within-subject ANOVA. There is no overall effect of GAN ($F$
...
- Repeated measure ANOVA pools data from participants to keep a single measurement (average) per experimental condition
  ...
  - **between-subject** factors are mutually exclusive (different people in those subgroups).
...
-   
  - Extra assumptions: sphericity ($J > 2$ groups)
  - If sphericity is violated, need to change our reference benchmark
