...
# Example of confounder: effect of money on elections
...
The **natural direct effect** is the expected change in $Y$ under treatment $x$ if $M$ is set to whatever value it would take under control $x^*$: $$\textsf{NDE}(x, x^*) = \mathsf{E}[Y\{x, M(x^*)\} - Y\{x^*,  M({x^*})\}].$$    


The **natural indirect effect** is the expected change in $Y$ if we set $X$ to its control value and change the mediator value which it would attain under $x$: $$\textsf{NIE}(x, x^*) = \mathsf{E}[Y\{x^*, M(x)\} - Y\{x^*,  M(x^*)\}].$$  
...
We define the **total effect** as $$\mathsf{TE}(x, x^*) = \textsf{NDE}(x, x^*) - \textsf{NIE}(x^*, x).$$
???
