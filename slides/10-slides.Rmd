...
.small[
There is no mean parameter for employees, but we can get the *best linear unbiased predictor*.
]

```{r}
#| eval: true
#| echo: false
#| fig-cap: "Catterpilar plot of best linear unbiased prediction of random effects with Wald confidence intervals."
#| label: fig-catterpilar
#| out-width: '70%'
#| fig-width: 8
#| fig-height: 2.5
ranef_mod <- ranef(rmod, condVar = TRUE)
d1 <- lattice::dotplot(ranef_mod)
d1$id$main <- NULL
d1$id
```
...
# Shrinkage or regularization effect

.small[
Predictions of random effects $\alpha_i$'s are shrunk towards zero (regularization effect).
]
