...
.box-6.medium.sp-after-half[Parametrization and testing]


.box-8.medium.sp-after-half[Contrasts]
...
.box-inv-4[Each cell contains the mean measurements of the subcategory]

---

# Why complete factorial designs?
...
.box-inv-4.sp-after[Cast problem<br>as a series of one-way ANOVA <br> vs simultaneous estimation]
...
.box-inv-4.sp-after[Graphical display = <br>**interaction plot**<br>]

.box-4.sp-after-half[sample mean per category]

.box-4.sp-after-half[measure of uncertainty <br>(1 std. error for mean, confidence interval, etc.)]



---

# Interaction plot with interaction
...
# Interaction plot without interaction

```{r interaction_plots1, echo = FALSE, fig.asp = 0.5, out.width = '80%', cache = TRUE}
set.seed(1234)
data_fake <- tibble::tibble(
  "ending" = factor(rep(c("happy", "sad"), each = 3)),
  "complexity" = factor(rep(c("complicated", "average", "simple"), 
...
.box-inv-5[More parameter than cells!]
...
.box-inv-5.small[
With $K$ replications per subgroup,<br>total sample size is $n = n_an_bK$
]
...
| residuals | $n-n_an_b$ | $\mathsf{MS}_{\text{res}}=\mathsf{SS}_{\text{res}}/ (n-ab)$ | |
| total | $n-1$ | | 
