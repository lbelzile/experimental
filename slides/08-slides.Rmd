...
Instead use standardized differences/measures or percentage of variability explained
...
The usual estimator $\widehat{d}_s$ uses sample average of groups and the pooled variance estimator $\widehat{\sigma}$.
...
For a one-way ANOVA, break down the variability $$\sigma^2_{\text{total}} = \sigma^2_{\text{resid}} + \sigma^2_{\text{effect}}$$ and define the percentage of variability explained by the $\text{effect}$.
...
Consider a completely randomized design with two factors $A$, $B$ and their interaction $AB$.
...
Similar formulae as the one-way case, with
...
.pull-right.box-inv-3.sp-after-half[
...
1. Compute the cutoff point: the value under $\mathscr{H}_0$ that leads to rejection at level $\alpha$. .box-inv-3.sp-before.sp-after[
