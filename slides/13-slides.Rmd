...
rownames(tabs) <- c("no", "yes")
colnames(tabs) <- c("integrated","sequential")
knitr::kable(tabs)
```
...
Counts are modelled (implicitly) as Poisson random variables.
...
- the null model, a simplification with fewer parameters than cells.
...
- the saturated model with $N=IJ$ cells/parameters and 
- the null additive model with $k=1 +(I-1) + (J-1)$ parameters
...
\end{align*}
with sum-to-zero constraints for $\alpha_i$, $\beta_j$. This has $1 + (I-1) + (J-1)$ parameters.
...
#| echo: false
#| label: tbl-counts
#| tbl-cap: "Purchase counts for Duke and Amir (2023)"
#| tbl-subcaption: ["expected (null)", "observed (alternative)"]
#| 
DA23_E2_long <- DA23_E2 |> 
  dplyr::group_by(format) |> 
  dplyr::summarize(no = sum(purchased == 0L),
            yes = sum(purchased)) |>
  tidyr::pivot_longer(cols = c("no","yes"), names_to = "purchased", values_to = "count") |>
  dplyr::mutate(purchased = factor(purchased))
mod0 <- glm(count ~ format + purchased, data = DA23_E2_long, family = "poisson")
m0 <- matrix(round(predict(mod0, type = "response"),2), nrow = 2)
row.names(m0) <- c("no", "yes")
colnames(m0) <- c("integrated","sequential")
knitr::kable(m0)
mod1 <- glm(count ~ format * purchased, data = DA23_E2_long, family = "poisson")
m1 <- matrix(predict(mod1, type = "response"), nrow = 2)
row.names(m1) <- c("no", "yes")
colnames(m1) <- c("integrated","sequential")
knitr::kable(m1)
```
...
- The standard machinery for comparing models
...
We can use a likelihood ratio test or score test
...
anova(mod_main, mod_satur, test = "LRT")  # likelihood ratio/deviance
anova(mod_main, mod_satur, test = "Rao")  # score test, aka Pearson X2 stat
```
