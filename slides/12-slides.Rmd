...
In an experiment, we can obtain the total effect via the ANOVA model, with $\tau= \mathrm{E}\{Y \mid \mathrm{do}(X=1)\} - \mathrm{E}\{Y \mid \mathrm{do}(X=0)\}$ the average treatment effect since
...
This approach has been discontinued, but still appears in papers.

Baron and Kenny recommended running the three linear regressions and testing
...
In large sample, $S \stackrel{\cdot}{\sim}\mathsf{No}(0,1)$, but this approximation is poor in small samples.

A popular way to estimate the _p_-value and the confidence interval is through the nonparametric **bootstrap** with the percentile method, popularized by @Preacher.Hayes:2004

Repeat $B$ times, say $B=10\ 000$
1. sample $n$ tuples tuples $(Y_i, X_i, M_i)$ from the database **with replacement** to obtain a new sample.
...
.pull-left[
.box-inv-4[Confidence interval]

Percentile-based method: for a equitailed $1-\alpha$ interval and the collection
...
.pull-right[
.box-inv-4[Two-sided _p_-value]

Compute the sample proportion of bootstrap statistics that are larger/smaller than zero.

Order bootstrap statistics $S^{(1)} \leq \cdots \leq S^{(B)}$ and find $M$ ($1 \leq M \leq B$) such that
If $S^{(M)} < 0 \leq S^{(M+1)}$ for $1 \leq M \leq B$. The $p$-value is then

$$p = 2\min\{M/B, 1-M/B\}$$

and zero if all values lie on either side of $0$.
...
- Careful with variables included in models (colliders)! 
 - *as-if* randomization assumption
 
- Generalizations to interactions, multiple mediators, etc. requires careful acknowledgement of confounding.
