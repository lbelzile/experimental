...
- Causal direction: $M \to Y$ — $M$ is an antecedent cause
...
$$
\underset{\text{average response of group $k$ at $w$}}{\mathrm{E}(Y \mid X=k, W=w)} = \underset{\text{intercept of group $k$}}{\alpha_0 +\alpha_k} + \underset{\text{slope of group $k$}}{(\beta_0 + \beta_k)} 
$$
...
Test jointly whether coefficients associated to $XW$, $\beta_1 = \cdots = \beta_K$ are zero. 
...
- The case where interactions between $XW \to M$ and/or $MW \to Y$ in a mediation analysis is called ``moderated mediation''.
