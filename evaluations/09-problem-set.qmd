...
Look at the [effect size and power examples](/example/effectsizepower.html); this should be helpful in completing the problem set.
...
1. Using the information from the quote, compute the effect size for Cohen's $f$ and $\widehat{\omega}^2$.^[In **R**, check out the function `F_to_omega2` and `F_to_f` from the `effectsize` package. You can also use the formula presented in the slides.] Note that you won't normally get the same value of Cohen's $f$ that is reported.
