...
1. Using @Oehlert:2010 approach
    a. Identify whether factors are crossed or nested.
    b. Determine whether factors should be fixed or random.
    c. Figure out which interactions can exist and whether they can be fitted.
...
4. Report the lab-specific variability  and comment on regional differences based on the predictions of the random effects.^[In SPSS, add `SOLUTION` to your `RANDOM` specification for the `lab` effect. In **R**, assuming you fit the model with the `lmerTest` or `lme4` package, we can extract the predictions of the lab specific mean differences from a fitted model via `lme4::ranef` function.]
5. Compute the correlation of measurements for different individuals in a given lab, following [the example 12.4 from the course notes](https://lbelzile.github.io/math80667a/mixed.html#exm-verbalization-memorization)
