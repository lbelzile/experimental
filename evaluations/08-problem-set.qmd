...
1. Using the information from the quote, compute the effect size $\widehat{\omega}^2$.^[In **R**, check out the function `F_to_omega2` from the `effectsize` package. You can also use the formula presented in the slides.] Also check that you get the same value of Cohen's $f$ (up to rounding) that is reported.
