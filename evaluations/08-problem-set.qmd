...
1. Compute the effect size $\widehat{\omega}^2$  from the quoted $F$ statistic. Also check that you get the same value of Cohen's $f$ (up to rounding) that is reported
2. Compute the overall sample size necessary to replicate this study with a power of at least $0.99$: compute the power for both the overall effect and the three pairwise $t$-tests based on the reported values of Cohen's $d$ and pick the maximum sample size needed.
...
4. Based on observation of Figure 1, why might one object to using estimated effect size reported in peer-reviewed papers?^[*Hint*: are the original effect size in line with the replications or not?]
